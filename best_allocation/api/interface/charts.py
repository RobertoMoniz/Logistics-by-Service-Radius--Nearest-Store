...
def get_hubs_coordinates(hub):
    hubs_df = pd.read_csv('data/suplementar/hubs locations.csv',sep=';')
...
def show_evolution(df,hub,tiles= "cartodbpositron",min_zoom=5, zoom_start=11, max_zoom=40,radius=3,min_speed=6):
    ...
    heat_df = df[df['HUB']==hub][['date','latitude','longitude']]
...
def show_heatmap(df,hub):
    ...
    heat_df=df[df['HUB']==hub][['latitude','longitude']]

    coordenadas = get_hubs_coordinates(hub)

    new_map = folium.Map(location=coordenadas,tiles= "cartodbpositron",min_zoom=5, zoom_start=11, max_zoom=40)
